...
export const LIST_TAGS_DESC = `Search the company tag directory by substring: id, name. Required: query — Kaiten caps an unfiltered tag list at 100 and ignores limit, so without a search term the tag you want is often simply absent. Use it to turn a tag name into the tag_ids filter for kaiten_search_cards. Not needed for kaiten_add_card_tags / kaiten_remove_card_tags — those take names directly. Not cached.`;
