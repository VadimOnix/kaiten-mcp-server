...
export const ADD_CARD_TAGS_DESC = `Attach one or more labels (tags) to a card by name. Required: card_id and tag_names (array, at least one). A tag that does not exist yet is created and linked; existing tags are matched by name. Issues one API call per name and continues on individual failures, returning a summary { card_id, succeeded, failed }. Idempotent — re-adding an existing tag is a no-op. Remove tags with kaiten_remove_card_tags; a card's current tags show in kaiten_get_card.`;

export const REMOVE_CARD_TAGS_DESC = `Detach one or more labels (tags) from a card by name. Required: card_id and tag_names (array, at least one), matched case-insensitively against the card's current tags. Resolves each name to its tag id, then unlinks it (the tag itself is not deleted globally). A name not present on the card is reported as a per-item failure. Returns { card_id, succeeded, failed }. See the card's tags via kaiten_get_card; add tags with kaiten_add_card_tags.`;

export const SEARCH_CARDS_DESC = `Primary discovery tool: find cards by text and filters, newest first, in a compact context-friendly format. Searches the default space (limit 10) unless you pass space_id (0 = ALL spaces, slow) or board_id (recommended, to keep results small and fast). query is a partial match on title/description/comments — for Russian text search by WORD ROOT, not the inflected form (see server instructions). Filters: column/lane/state/type, owner/member/tag IDs, ISO-8601 date ranges (created/updated/due), asap/overdue flags, condition (1=active default, 2=archived). limit ≤20, skip paginates, verbosity minimal|normal|detailed. Returns compact rows — use kaiten_get_card for full detail.`;
