...
export const CreateCardSchema = z.object({
  ...
  size: z.number().min(0).optional().describe('Card estimate (optional). Written as Kaiten size_text; errors if the board rejects it'),
  size_unit: z.string().min(1).max(20).optional().describe('Estimate unit appended to size, e.g. "SP" or "h" (optional)'),
...
export const UpdateCardSchema = z.object({
  ...
  size: z.number().min(0).optional().describe('New card estimate (optional). Written as Kaiten size_text; errors if the board rejects it'),
  size_unit: z.string().min(1).max(20).optional().describe('Estimate unit appended to size, e.g. "SP" or "h" (optional)'),
  asap: z.boolean().optional().describe('Mark as ASAP (optional)'),
