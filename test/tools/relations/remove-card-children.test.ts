...
import { removeCardChildren } from '../../../src/tools/relations/remove-card-children.js';
...
describe('kaiten_remove_card_children tool module', () => {
  it('all-success: parent_card_id top key, child_card_id item key, "removed" verb', async () => {
    const removeCardChild = vi.fn().mockResolvedValue({ id: 5 });
    const res = await removeCardChildren.run(
      { card_id: 5, child_card_ids: [42, 43] },
      fakeCtx({ client: { removeCardChild } }),
    );
    ...
    expect(removeCardChild).toHaveBeenNthCalledWith(1, 5, 42, undefined);
    expect(removeCardChild).toHaveBeenNthCalledWith(2, 5, 43, undefined);
  });

  it('partial-failure: NOT isError, failed item carries child_card_id key', async () => {
    const removeCardChild = vi
      .fn()
  ...
  it('all-failure: isError true', async () => {
    const removeCardChild = vi.fn().mockRejectedValue(new Error('nope'));
    const res = await removeCardChildren.run(
  ...
  it('rejects invalid args via the schema', async () => {
    const res = await removeCardChildren.run({ card_id: 5, child_card_ids: [] }, fakeCtx());
    expect(res.isError).toBe(true);
    expect(res.content[0].text).toContain('VALIDATION_ERROR');
  });

  it('has expected metadata', () => {
    expect(removeCardChildren.name).toBe('kaiten_remove_card_children');
    expect(removeCardChildren.description.length).toBeGreaterThan(0);
  });
});
